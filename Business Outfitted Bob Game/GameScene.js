// keep count of Bob's earnings
let score = 0;
// multiply money
const moneyMultiplier = 100;
// control speed of Bob sprite
let speed = 1;
...
class GameScene extends Phaser.Scene {
  ...
  update() {
    ...
    // Code to check whether any of the arrow keys were pressed, move Bob
    if (rightArrow) {
      moveBobRight();
    } else if (leftArrow) {
      moveBobLeft();
    ...
    // Variables that store the x and y coordinates of the Bob sprite
    const bobXCoord = gameState.player.x;
    const bobYCoord = gameState.player.y;

    // Code to check collision between Bob and edges of the canvas of the game
